...
<body>
	...
	<?php 
		if (isset($_POST['enviar'])) {
			...
			$sql= "update profesor set nombre='".$nombre."', correo='".$correo."',usuario='".$usuario."',password='".$password."'where id='".$id."'";
			$resultado = mysqli_query($conex,$sql);
			if ($resultado ) {
				echo "<script language='JavaScript'> alert('Los datos fueron actualizados de forma correcta');
				location.assign('editar_tabla_profesor.php')
				</script>";
			}else{
	...
	<form action="<?=$_SERVER['PHP_SELF']?>" method="post">
		...
		<a href="editar_tabla_profesor.php">Regresar</a>
	</form>
	<?php 
		}
	 ?>
